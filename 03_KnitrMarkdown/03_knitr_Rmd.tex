...
\item Pass arguments to the graphics device via the chunk
  option {\tt dev.args}.
\item To increase the size of axis labels, I seem to need {\tt
  par(cex.lab=1.5)} as well as {\tt dev.args=list(pointsize=18)}
\ei
}
...
\note{Graphics in KnitR are super easy. For the most part, you don't
  ...
  As far as I can tell, it seems like, to change the size of axis
  labels, you can't just use the pointsize device argument; you also
  need to use something like {\tt par(cex.lab=1.5)}.
}
\end{frame}
