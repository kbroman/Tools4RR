...
\note{
  I can't really hope that to teach you Python in 50 minutes, but I'll
...
\note{
  ...
  and then import the module and run some of the functions by hand.

  The {\tt read.markers} function reads in the ordered list of markers
  as a vector. Vectors in Python are indexed starting at 0. You can
  use the {\tt len} function (like {\tt length()} in R) to get the
  length.
...
\note{
  ...
  The first line (between the triple single-quotes) is a description
  that will be shown if you import the module and type
  {\tt help(convert2.read\_markers)}.
  The triple single-quotes allow the string to go across multiple
  lines. It's not necessary here. Strings in Python can be defined
  using single- or double-quotes, just like in R.
  ...
  one-line {\tt for} loop, which just applies the {\tt strip()}
...
\note{
  ...
  an instance of a class (the data object for a single subject).

  Within that function, {\tt self} refers to the newly defined
  instance of the class, and {\tt self.family}, etc., are the way to
  refer to the elements of the class.

  I'd define a {\tt Person} object by calling
...
\note{
  ...
  I use {\tt with open() as file:} to open the file again. If anything
  ...
  indices.  I'm going to create a hash of {\tt Person} objects,
  indicated by strings like {\tt "1-2"} for individual {\tt 2} in
  family {\tt 1}.
...
\note{
  ...
  The {\tt re.sub} function is for doing text substitutions with
  regular expressions. In ``{\tt r'{\textbackslash}s+'}'' the {\tt r}
  means raw text and the {\tt {\textbackslash}s+} means any amount of
  white space.
  ...
  the marker name (getting rid of any blank characters) and then go
  through the rest of the line, 7 characters at a time.
  ...
  The function doesn't return anything, because it modifies the input
  {\tt person} object (as a side effect).
}
\end{frame}
...
\note{
  ...
  {\tt writeln} is just a little wrapper for the {\tt file.write}
