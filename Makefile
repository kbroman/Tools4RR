...
homework: ${HW}/01_homework.pdf ${HW}/02_homework.pdf ${HW}/02_hw_solutions.pdf ${HW}/02_homework.R ${HW}/03_homework.pdf ${HW}/03_hw_solutions.Rmd ${HW}/03_hw_solutions.html $(HW)/04_homework.pdf $(HW)/06_homework.pdf $(HW)/07_homework.pdf $(HW)/08_homework.pdf $(HW)/09_homework.pdf
...
${HW}/07%.pdf: ../07_ClearCode/07%.pdf
	cp $< $@

${LEC}/08%.pdf: ../08_Rpack/08%.pdf
	cp $< $@

${HW}/08%.pdf: ../08_Rpack/08%.pdf
	cp $< $@

${LEC}/09%.pdf: ../09_TestingDebugging/09%.pdf
	cp $< $@

${HW}/09%.pdf: ../09_TestingDebugging/09%.pdf
	cp $< $@
