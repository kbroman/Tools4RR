...
\note{
  This is from Hadley's paper on his {\tt testthat} package.
}
\end{frame}
...
\note{
  The {\tt testthat} package simplies unit testing of code in
  R packages.
...
\item Run {\tt test()}
 ...
 \ei
\item Consider {\tt auto\_test("R", "inst/tests")}
 \bi
...
\item Focus on the {\hilit boundaries}
   ...
   \item Things exactly matching; things with no matches.
   \ei
\item Test handling of missing data.
   \bi
   \item {\tt NA}, {\tt Inf}, {\tt -Inf}
   \ei
\item Automate the construction of test cases, or at least create a
  table of inputs and expected outputs and then run through the values
  in the table.
\ei

\note{
  You want your code to produce the correct output for {\nhilit any}
  input, but you can't test all possible inputs, and you don't really
  need to.

  This is an experimental design question: what is the minimal set of
  inputs to verify, with little uncertainty, that the code is correct?

  We generally focus on boundary conditions, as those tend to be the
